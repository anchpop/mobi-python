    ...
    def readRecord(self, recordnum, disable_compression=False):
        if self.config:
            if self.config['palmdoc']['Compression'] == 1 or disable_compression:
                ...
                n = recordnum
                result = uncompress_lz77(self.contents[self.records[n]['record Data Offset']:self.records[
                                         n + 1]['record Data Offset'] - self.config['mobi']['extra bytes']])
                print("noice,", result)
                return result
...
###########  Private API ###########################
    ...
    def __iter__(self):
        if not self.config:
            return
        print('First Non-book index', self.config['mobi']['First Non-book index'])
